...
yTrain = yCategorical(training(cv));
XVal = X(test(cv), :);
yVal = yCategorical(test(cv));

% Define neural network architecture
layers = [
    featureInputLayer(fftLength, 'Normalization', 'zscore') % Input layer matching FFT length
    fullyConnectedLayer(128)
    reluLayer
    fullyConnectedLayer(64)
    reluLayer
    fullyConnectedLayer(numClasses) % Output layer with neurons equal to class count
    softmaxLayer
    classificationLayer
];
